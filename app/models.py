...
class Item(Model):
    __tablename__ = 'item'
    id = Column(Integer, primary_key=True)
    userID = Column(Integer, ForeignKey('ab_user.id'), nullable=False, primary_key=True)
    ...
    @property
    def serialize(self):
        return {'id': self.id, 'modified_at':dump_datetime(self.modified_at),'many2many':self.serialize_many2many}
